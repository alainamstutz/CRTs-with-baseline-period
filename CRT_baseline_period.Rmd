...
So, a reasonable site-level variance for your binary outcome is about 0.365.

If there’s additional within-site variation over time, we include σ^2cp.
Typically, this is a fraction of σ^2c, e.g., half the site-level variance, 0.183.


Let’s assume the control group has a baseline PrEP uptake of 40% (p0=0.40). The treatment group increases this by 20 percentage points to p1=0.60.
OR = (0.60 / (1−0.60)) / (0.40 / (1−0.40)) = 1.5 / 0.6667 = 2.25
δ1=log(2.25) ≈ 0.81
So, instead of 2.4 * rx, we should use 0.81 * rx in the model.

```{r}
simple_crt_binary <- function(nsites, n) {
  # Treatment assignment (1:1 randomization)
  defC <- defData(varname = "rx", formula = "1;1", dist = "trtAssign")
  
  # Define Site-Level Random Effect (c) with variance = 0.365
  defC <- defData(defC, varname = "c", formula = "0", variance = 0.365, dist = "normal")
  
  # Define Individual-Level Outcome (logit model)
  # Treatment effect is log(OR) = 0.81 (as explained before)
...
# Generate data for 20 sites, each with 50 individuals
dd_binary <- simple_crt_binary(20, 50)
...
```{r}
dd_binary <- simple_crt_binary(200,100)
...
```{r}
replicate <- function() {
  dd_binary <- simple_crt_binary(30,10)
  fit_logistic <- glmer(y ~ rx + (1 | site), data = dd_binary, family = binomial)
